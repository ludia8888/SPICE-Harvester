...
- 따라서 핵심 구현은 3개:
  ...
  3) **E2E Policy Evals**: enterprise error taxonomy → LangGraph edge/최종 상태가 절대 흔들리지 않게 회귀 테스트로 고정
...
### 2.1 Agent Runtime (LangGraph Executor)

- 순차 step executor + retry 분기: `backend/agent/services/agent_graph.py`
...
검증: LangGraph가 어떤 edge를 타고 어떤 `final_state`가 되는지 고정
