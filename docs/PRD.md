...
RQ-OD-001 BackingDataSource 연결 선언
	...
	•	E2E Proven: `make backend-prod-full` (OpenAPI smoke 경유) — 부분 검증
...
RQ-OD-002 PropertyMapping 자동/수동 매핑
	...
	•	Validated: `backend/bff/routers/objectify.py` (source/target/type preflight), `backend/bff/routers/object_types.py` (auto_generate_mapping)
	•	Enforced: `backend/objectify_worker/main.py` (mapping spec/필수 필드/타입 검증)
	•	E2E Proven: `backend/bff/tests/test_objectify_mapping_spec_preflight.py` (unit), `make backend-prod-full` (부분)
...
RQ-OD-003 스키마 호환성 검증(하드게이트)
	...
	•	E2E Proven: 미검증 (unit: `backend/tests/unit/workers/test_objectify_worker_p0_gates.py`)
...
RQ-OD-006 Indexing Pipeline 자동 연결
	...
	•	E2E Proven: `make backend-prod-full` (core flow)

RQ-OD-007 스키마 변경 감지/마이그레이션 게이트
	•	Defined: `backend/pipeline_worker/main.py` (schema_hash mismatch + schema_diff)
	...
	•	E2E Proven: 부분(게이트 기록은 생산 테스트 간접 검증)

RQ-OD-008 Writeback/Edits 마이그레이션 정책
	•	Defined: `backend/shared/services/dataset_registry.py` (instance_edits), `backend/instance_worker/main.py` (edit 기록)
	•	Validated: pk 변경 시 reset_edits 없으면 409 (`backend/bff/routers/object_types.py`)
	•	Enforced: reset_edits 시 편집 이력 삭제 (`clear_instance_edits`)
	•	E2E Proven: 미검증(추가 시나리오 테스트 필요)
...
RQ-RS-001 LinkType 1급 리소스
	...
	•	E2E Proven: `make backend-prod-full` (OpenAPI smoke 경유)
...
RQ-RS-002/003 FK/Join 관계 정의
	...
	•	E2E Proven: 미검증(링크 인덱싱 E2E 필요)

RQ-RS-007 LinkIndexing Pipeline
	•	Defined: `backend/objectify_worker/main.py` (_run_link_index_job)
	•	Validated: pk/row key/relationship 값 검증
	•	Enforced: dangling_policy=FAIL 시 전체 실패 (0건)
	•	E2E Proven: 미검증(추가 시나리오 테스트 필요)

RQ-RS-008 Dangling Reference 정책
	•	Defined: link_index options.dangling_policy
	•	Validated: WARN/FAIL 분기
...
RQ-SEP-001/003 계층 분리 + 자동 반영
	...
	•	E2E Proven: 부분(`make backend-prod-full` 코어 플로우)
...
RQ-SEP-005 AccessPolicy (행/컬럼 마스킹)
	...
	•	Enforced: `backend/bff/routers/query.py`, `backend/bff/routers/graph.py` (마스킹/필터 적용)
	•	E2E Proven: 미검증 (unit: `backend/tests/unit/utils/test_access_policy.py`)
