...
Pipeline Builder는 Join/Union 같은 변환을 제공하며, Union은 스키마 동일성을 요구하고 불일치 시 누락 컬럼 목록을 포함한 에러를 보여줍니다.  ￼
	...
	•	합격: 렌더링만 바뀌고, 로직 자체는 변경되지 않음(의사코드 편집은 불가라는 점까지 상용 플랫폼과 유사).  ￼
	•	☑ 커스텀 로직 확장: UDF(사용자 정의 함수) 버전 참조형 실행(`udfId`, optional `udfVersion`)이 가능.
	•	검증: Python UDF 생성 → 파이프라인 재사용 → 버전 변경/업그레이드 시 preview/build parity 확인.
	•	합격: `udfCode` 인라인 실행은 차단되고, 참조형 UDF 계약만 허용(거버넌스/재현성 강화).  ￼
...
또한 스트리밍 파이프라인에서는 Flink 기반으로 묶여 실행된다는 설명(잡 그룹)도 존재합니다.  ￼
	...
	•	☑ Batch Incremental 모드: 정책 기반 default 해석(`default`는 additive 신호 없으면 `snapshot_replace`).
	•	검증: additive 신호 없음/false/true 케이스별로 resolved write_mode가 fail-closed 규칙대로 고정되는지 확인.
	•	합격: default 모드가 임의 추정으로 append되지 않으며, 정책 해시로 build↔deploy 일관성 검증.  ￼
...
Deploy는 로직을 업데이트, Build는 로직을 실행해 결과를 물리화라고 구분하며, 비용 때문에 “deploy만 하고 build는 나중”이 가능하다고 설명합니다.  ￼
	...
	•	증거:
		...
		- docs/platform_checklist/PIPELINE_ARTIFACT_E2E.md
...
외부 레퍼런스(상용 플랫폼)의 Pipeline management:
	•	파라미터 & 커스텀 함수로 로직 재사용  ￼
...
또한 Build settings에서 배치·스트리밍 파이프라인의 성능 설정/컴퓨트 프로파일을 조정할 수 있습니다.  ￼
	...
	•	☐ 저지연(특히 15분 이하) 워크로드 최적화 옵션(“faster pipelines” 포지션에 대응).
...
Data Lineage를 별도 앱으로 제공하며, 데이터가 플랫폼 내에서 어떻게 흐르는지(조상/자손 확장, 스키마/마지막 빌드/코드 드릴다운, out-of-date 표시 등) 확인할 수 있고 스냅샷 공유도 가능합니다.  ￼
...
19) 보안/감사(Audit)·거버넌스(엔터프라이즈급 주장에 사실상 필수) (P0~P1)
