...
- Population: files **662**, functions **6343**, classes **895**, internal cross-imports **1579**

| # | Check | Ratio | Target | Status | Metric Basis |
| --- | --- | --- | --- | --- | --- |
| 1 | 계층 간 누수 | 0/1579 (0.00%) | <= 0.50% | **PASS** | `layer_leak_imports / internal_cross_imports` |
...
| 5 | 파일 응집도 저하 | 51/662 (7.70%) | <= 20.00% | **PASS** | `cohesion_risk_files / files` |
| 6 | 파일 단일 책임 위반 | 60/662 (9.06%) | <= 12.00% | **PASS** | `single_responsibility_risk_files / files` |
| 7 | 함수 단일 책임 위반 | 326/6343 (5.14%) | <= 10.00% | **PASS** | `(cc>=25 or len>=120) / functions` |
| 8 | 연속 상속 깊이(>=3) | 15/895 (1.68%) | <= 5.00% | **PASS** | `classes_depth>=3 / classes` |
| 9 | 복잡도 과다(CC>=15) | 732/6343 (11.54%) | <= 15.00% | **PASS** | `cc>=15 / functions` |
| 10 | 롱메서드(len>=80) | 415/6343 (6.54%) | <= 8.00% | **PASS** | `len>=80 / functions` |
