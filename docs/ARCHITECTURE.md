...
- Agent Plan은 별도 제어면(BFF)에서 컴파일/검증/승인 후 실행되며, write는 `simulate→approve→submit`을 서버가 강제한다. (`/api/v1/agent-plans/*`)
- Pipeline Agent(자연어 ETL)는 `POST /api/v1/agent/pipeline-runs`로 실행되며, 내부적으로 BFF의 Pipeline Plans API를 호출해 **plan compile/preview/repair/spec 준비**를 오케스트레이션한다. (상세: `docs/PIPELINE_AGENT.md`)
- Pipeline Agent는 overreach 방지를 위해 `PipelineTaskSpec`(scope/allow flags)을 먼저 확정하고, 그 범위 내에서만 join/cleansing/advanced/specs를 수행한다.
- Pipeline Plans는 필요 시 MCP 기반 플래너로 전환 가능하며(feature flags), LLM은 DSL 문자열이 아니라 **plan-builder tool call script**를 생성한다.
...
- Pipeline Agent graph: `backend/agent/services/pipeline_agent_graph.py`, `backend/agent/routers/pipeline_agent.py`
- PipelineTaskSpec(스코프 계약): `backend/shared/models/pipeline_task_spec.py`, `backend/shared/services/pipeline_task_spec_policy.py`, `backend/bff/services/pipeline_task_spec_agent.py`
- Pipeline Plans API: `backend/bff/routers/pipeline_plans.py`, `backend/bff/services/pipeline_plan_compiler.py`, `backend/bff/services/pipeline_plan_validation.py`
...
LangGraph 기반 실행을 재현/감사하기 위한 control-plane 저장소를 Postgres에 둔다.
