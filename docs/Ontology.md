...
아래는 외부 상용 플랫폼 문서에 나온 정의/제약만으로, “온톨로지 타입(설계도 부품들)”, “프로퍼티 기본 데이터 타입(Base types)”, “Value type(의미 래퍼)”를 분해해서 설명한 것입니다.

⸻

0) 먼저, 외부 플랫폼 문서가 말하는 “타입”의 큰 분류

문서는 타입을 크게 두 범주로 나눕니다.  ￼
...
문서에서 “Ontology라는 아티팩트가 저장하는 리소스”로 공식적으로 나열된 항목은 Object types / Link types / Action types / Interfaces / Shared properties / Object type groups 입니다.  ￼
...
즉, 해당 플랫폼에서 property는 단순 컬럼이 아니라 객체의 정체성/표현을 결정하는 요소입니다.
...
→ 즉, 해당 플랫폼에서 “관계 변경”은 결국 키/속성 변경과 결합되어 모델링됩니다.
...
→ 이게 벤더식 “Hard gate(실패하면 못 바꿈)”의 대표적인 구현 축입니다.
...
외부 플랫폼의 properties 문서에는 base type별 지원/제약이 표 형태로 정리되어 있고, 특히 Array/Struct의 제약이 명시돼 있습니다.  ￼
...
2-1) 기본 스칼라(원자) 타입
	...
	•	Date / Timestamp
(이 목록은 properties/base types 체계에서 일반적으로 다루는 primitive군이며, 문서에서는 data types가 RDF/OWL/XSD에서 영감을 받았다고 설명합니다.)  ￼
...
다시 강조하지만, 문서에서는 value type 버전에서:
...
즉, “ETL 단계에서 의미 타입을 주입한 뒤 → 온톨로지에 기록”이라는 설계가 해당 플랫폼에서도 가능한 형태로 드러납니다.
...
알겠습니다, 시현님. **Action type(액션 타입)**은 상용 온톨로지 플랫폼에서 “사용자가 버튼 한 번으로 일으키는 업무 트랜잭션”을 정의(설계)하는 타입입니다. 단순히 “속성 하나 수정”이 아니라, 여러 객체/속성/링크를 한 번에 바꾸고 + 제출 시 부수효과까지 포함하는 것이 핵심입니다.  ￼
...
문서는 “Ontology rule은 Ontology의 특정 요소를 바꾸며, 기존 타입의 objects/links를 생성·수정·삭제할 수 있다”고 못 박습니다.  ￼

여기서 중요한 세부 규칙이 하나 있습니다:
	•	1:1 또는 1:N 링크를 생성/삭제하려면 “link rule만으로 끝나는 게 아니라”, object rule로 ‘foreign key property’를 수정해야 한다고 명시합니다.  ￼

이 문장 하나가 굉장히 실무적입니다.
즉, 해당 플랫폼에서 “관계(link)”는 그냥 선을 그리는 게 아니라, 데이터 모델 상 FK(키) 갱신과 결합된 방식으로 안전하게 관리됩니다.
...
한국어 문서(웹훅)에는 실행 순서도 나옵니다:
	...
	•	웹훅을 부작용(side effect) 으로 설정하면 “규칙 평가 후 실행”, 사용자에게는 플랫폼 오브젝트 수정 후 성공 메시지가 먼저 보일 수 있고, 부작용은 그 뒤에 실행될 수도 있다고 설명합니다.  ￼
...
Action은 결국 “사용자가 데이터를 편집하는 행위”인데, 이 구조는 이를 원본 backing dataset에 바로 쓰지 않고 별도의 writeback dataset으로 적재하는 구조를 권장/강제합니다.  ￼
...
외부 SDK 문서에서는 액션 적용 후 Object Storage V1의 변경은 eventually consistent일 수 있어 반영에 시간이 걸릴 수 있다고 명시합니다.  ￼
