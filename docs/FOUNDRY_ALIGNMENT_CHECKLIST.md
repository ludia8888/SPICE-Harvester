...
- Connectivity Update Export Settings For Connection (v2): https://www.palantir.com/docs/foundry/api/v2/connectivity-v2-resources/connections/update-export-settings-for-connection
- Connectivity Upload Custom JDBC Drivers Connection (v2): https://www.palantir.com/docs/foundry/api/v2/connectivity-v2-resources/connections/upload-custom-jdbc-drivers-connection
...
- [x] Foundry v2 connectivity connection configuration surfaces are exposed: `GET /api/v2/connectivity/connections/{connectionRid}/getConfiguration`, `POST /api/v2/connectivity/connections/getConfigurationBatch`, `POST /api/v2/connectivity/connections/{connectionRid}/updateSecrets`, `POST /api/v2/connectivity/connections/{connectionRid}/updateExportSettings`, `POST /api/v2/connectivity/connections/{connectionRid}/uploadCustomJdbcDrivers`.
- [x] Connectivity connection 응답은 Foundry 문서 필드(`connectionConfiguration`, `parentFolderRid`, `exportSettings`)를 기본으로 반환하며, `exportSettings`는 문서 필드(`exportsEnabled`, `exportEnabledWithoutMarkingsValidation`)로 정규화됩니다. 내부 호환을 위해 `configuration` alias를 병행 유지합니다.
...
- [x] Connectivity connector coverage includes `google_sheets + snowflake + postgresql + mysql + sqlserver + oracle`, with source-type mapping `google_sheets_connection|snowflake_connection|postgresql_connection|mysql_connection|sqlserver_connection|oracle_connection` and table-import source mapping `google_sheets|snowflake_table_import|postgresql_table_import|mysql_table_import|sqlserver_table_import|oracle_table_import`.
...
- [x] Connectivity execute surfaces are complete for preview resources: `fileImports/{fileImportRid}/execute` uses file-import runtime dispatch, and `virtualTables/{virtualTableRid}/execute` is exposed with build run persistence.
...
- [x] Connectivity delete semantics are strict hard-delete: `connections`/`tableImports`/`fileImports`/`virtualTables` delete operations remove registry sources directly (FK cascade cleanup), and no soft-disable fallback path remains in runtime handlers.
- [x] Connectivity runtime resolves connector kinds/import-config keys in strict mode (unknown `source_type` no longer falls back to `google_sheets`/`table_import_config`).
...
- [x] Dataset `files` list returns Foundry error envelope on lakeFS unavailability (instead of silent empty list fallback).
- [x] Pipeline scheduler runtime respects paused state (`status=paused`) and skips paused pipelines even when schedule is due.
