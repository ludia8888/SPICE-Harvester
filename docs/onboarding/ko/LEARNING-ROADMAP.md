...
> 이 문서는 Spice OS에 처음 합류한 개발자가 **4주 만에 자립적으로 코드를 작성**할 수 있도록 안내하는 학습 로드맵입니다. 매일 1~2시간씩 투자하는 것을 기준으로 합니다.
...
**체크포인트:** `curl http://localhost:8002/api/v1/health`가 `"healthy"` 응답 반환

### Day 2: 제품 이해

- [ ] [이 제품이 뭔가요?](01-WHAT-IS-SPICE.md) 읽기 (20분)
- [ ] [멘탈 모델](02-MENTAL-MODEL.md) 읽기 (30분)
- [ ] 기존 docs-portal 한국어 문서 훑어보기:
  - [ ] docs-portal 핵심 개념 문서 (20분)

**체크포인트:** 동료에게 "이 프로젝트가 뭘 하는 건지" 1분 안에 설명할 수 있음

### Day 3: 첫 API 호출

- [ ] [첫 API 호출](04-FIRST-API-CALL.md) 따라하기 (60분)
- [ ] 데이터베이스 생성 → 객체 유형 정의 → 인스턴스 생성 → 검색
- [ ] 각 단계에서 "뒤에서 무슨 일이 일어나는가"를 이해

**체크포인트:** curl로 직접 인스턴스를 생성하고 검색할 수 있음

### Day 4: 아키텍처 이해

- [ ] [아키텍처 이해하기](05-ARCHITECTURE-EXPLAINED.md) Level 1, Level 2 읽기 (40분)
- [ ] 4개 레이어 구조를 이해
- [ ] Read Path vs Write Path 차이 이해
- [ ] 기존 docs-portal 문서:
  - [ ] docs-portal 아키텍처 투어 문서 (30분)

**체크포인트:** "요청이 BFF → OMS → ES를 거쳐 응답된다"를 그림으로 그릴 수 있음

### Day 5: 프론트엔드 둘러보기

- [ ] [프론트엔드 둘러보기](07-FRONTEND-TOUR.md) 읽기 (30분)
- [ ] 프론트엔드 로컬 실행 (`cd frontend && npm run dev`)
- [ ] 주요 페이지 5개 직접 클릭하며 둘러보기
  - Overview, Databases, Ontology, Instances, Pipeline Builder
- [ ] 브라우저 DevTools Network 탭 열고 API 호출 관찰

**체크포인트:** "이 UI 페이지는 어떤 백엔드 API를 호출한다"를 3개 이상 말할 수 있음
...
**체크포인트:** BFF에 어떤 라우터가 있고, 라우터가 어떤 구조인지 설명할 수 있음
...
**체크포인트:** OMS가 BFF와 어떻게 다른지, 어떤 역할을 하는지 설명할 수 있음
...
**체크포인트:** 검색 요청이 BFF의 어떤 파일 → OMS의 어떤 파일을 거치는지 알 수 있음

### Day 9: 공유 라이브러리 + 워커 읽기
...
**체크포인트:** Worker가 Kafka에서 메시지를 받아 어떻게 처리하는지 설명할 수 있음
...
**체크포인트:** 특정 테스트 파일 하나를 지정하여 단독 실행할 수 있음
...
- [ ] 간단한 연습: 기존 API 엔드포인트의 응답에 필드 하나 추가해보기
  1. 모델에 필드 추가
  2. 서비스에서 값 채우기
  3. 테스트 수정

**체크포인트:** 코드 변경 → 테스트 실행 → 통과를 스스로 완료할 수 있음

### Day 12: 단위 테스트 작성

- [ ] 기존 서비스 함수에 대한 단위 테스트 2~3개 작성
- [ ] Mock 사용법 익히기 (AsyncMock, MagicMock)
- [ ] `PYTHONPATH=backend python3 -m pytest` 으로 내 테스트만 실행

**체크포인트:** 새 테스트 파일을 만들고 통과시킬 수 있음

### Day 13: 파이프라인 개념 이해

- [ ] docs-portal의 파이프라인 도구 레퍼런스 읽기
- [ ] 프론트엔드 Pipeline Builder 페이지에서 간단한 파이프라인 만들어보기
- [ ] `backend/pipeline_worker/main.py` 읽기

**체크포인트:** 파이프라인 노드(Input/Transform/Output)의 개념을 이해

### Day 14: 커넥터/데이터 임포트 이해

- [ ] `backend/data_connector/` 코드 구조 읽기
- [ ] 지원하는 커넥터 타입 6가지 확인
- [ ] Import Mode (SNAPSHOT, APPEND, INCREMENTAL, CDC, UPDATE) 이해

**체크포인트:** 외부 DB 데이터가 어떻게 Spice OS로 들어오는지 설명할 수 있음

### Day 15: Event Sourcing 심화

- [ ] docs-portal의 이벤트 소싱 한국어 문서 읽기
- [ ] `backend/shared/models/event_envelope.py` 읽기
- [ ] MinIO Console에서 실제 저장된 이벤트 확인 (http://localhost:9001)
- [ ] [아키텍처 이해하기](05-ARCHITECTURE-EXPLAINED.md) Level 3 읽기

**체크포인트:** "왜 Event Sourcing을 쓰는지"를 3가지 이유로 설명할 수 있음
...
- [ ] 다른 팀원의 PR 코드 리뷰 참여
- [ ] 코드 스타일, 테스트 커버리지, 아키텍처 패턴 확인

**체크포인트:** 첫 PR을 제출하고 머지할 수 있음
...
- [ ] E2E 테스트 작성 시도
- [ ] 모니터링 도구 사용 (Grafana: http://localhost:13000, Jaeger: http://localhost:16686)
- [ ] 다른 신규 팀원의 온보딩 도움주기 (가르치면서 배우기)
- [ ] [트러블슈팅 FAQ](10-TROUBLESHOOTING-FAQ.md)에 새로 발견한 문제 추가

---

## 핵심 파일 읽기 우선순위 (Top 15)

빠르게 코드를 이해하고 싶다면, 이 순서로 파일을 읽으세요:
