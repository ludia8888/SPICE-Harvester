...
- Pipeline MCP server: plan-builder + 분석(키/타입/preview/eval/inspect) 도구 제공
  - 코드: `backend/mcp_servers/pipeline_mcp_server.py`
...
- MCP tool server: `backend/mcp_servers/pipeline_mcp_server.py`
...
- Docker에서는 `backend/mcp_servers/mcp-config.docker.json`을 `/app/mcp-config.json`로 마운트합니다.
