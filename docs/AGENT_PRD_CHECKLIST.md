...
## 4.2) Pipeline Agent (Data Pipeline)

| ID | Status | Evidence (code/tests/docs) | Gap / Next |
|---|---|---|---|
| PIP-001 | PARTIAL | `backend/bff/services/pipeline_agent_autonomous_loop.py`, `backend/bff/routers/agent_proxy.py`, `backend/bff/services/pipeline_plan_validation.py`, `backend/shared/services/pipeline_executor.py` | end-to-end run 검증(실데이터) 리그레션 필요 |
| PIP-002 | DONE | `backend/shared/services/pipeline_preview_inspector.py`, `backend/bff/services/pipeline_cleansing_utils.py`, `backend/tests/unit/services/test_pipeline_preview_inspector.py` | |
...
| PIP-004 | PARTIAL | `backend/bff/services/pipeline_join_evaluator.py`, `backend/bff/routers/pipeline_plans.py`, `backend/mcp/pipeline_mcp_server.py` | join 평가 지표 정합성 테스트 추가 필요 |
| PIP-005 | PARTIAL | `backend/bff/services/pipeline_agent_autonomous_loop.py`, `backend/mcp/pipeline_mcp_server.py`, `docs/PIPELINE_AGENT.md` | ontology/specs 경로는 별도 tool로 재도입 필요 |
...
| PIP-008 | REMOVED | `backend/shared/models/pipeline_task_spec.py`, `backend/shared/services/pipeline_task_spec_policy.py` | TaskSpec 추론/서브에이전트 경로 제거(단일 loop에서 scope를 직접 결정) |
| PIP-009 | DONE | `backend/shared/services/pipeline_plan_builder.py`, `backend/mcp/pipeline_mcp_server.py`, `backend/bff/services/pipeline_plan_autonomous_compiler.py`, `backend/bff/services/pipeline_agent_autonomous_loop.py`, `backend/shared/config/settings.py`, `docs/PIPELINE_AGENT.md` | |
