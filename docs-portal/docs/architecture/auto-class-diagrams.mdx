...
Class diagram of the core backend architecture including domain models, service layer, type system, and middleware.
...
classDiagram
    ...
    class OntologyManagementService {
        +OntologyRegistryService registry_service
    ...
    FastAPIApplication --> FunnelService : uses (internal runtime)
    FastAPIApplication --> OntologyManagementService : uses (Port 8000)
    OntologyManagementService --> OntologyRegistryService : uses
