...
This page traces the lifecycle of a request through Spice OS, from client submission to response delivery. Understanding data flow is essential for debugging, performance tuning, and capacity planning.
...
1. A pipeline definition is submitted through **BFF** to the pipeline orchestration services.
2. The orchestrator validates the pipeline configuration and stores it in PostgreSQL.
3. The orchestrator publishes a `pipeline.submitted` event to Kafka.
