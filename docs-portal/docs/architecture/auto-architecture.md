...
- Population: files **637**, functions **6095**, classes **884**, internal cross-imports **1514**

| # | Check | Ratio | Target | Status | Metric Basis |
| --- | --- | --- | --- | --- | --- |
| 1 | 계층 간 누수 | 0/1514 (0.00%) | ≤ 0.50% | **PASS** | `layer_leak_imports / internal_cross_imports` |
...
| 5 | 파일 응집도 저하 | 51/637 (8.01%) | ≤ 20.00% | **PASS** | `cohesion_risk_files / files` |
| 6 | 파일 단일 책임 위반 | 58/637 (9.11%) | ≤ 12.00% | **PASS** | `single_responsibility_risk_files / files` |
| 7 | 함수 단일 책임 위반 | 329/6095 (5.40%) | ≤ 10.00% | **PASS** | `(cc>=25 or len>=120) / functions` |
| 8 | 연속 상속 깊이(>=3) | 15/884 (1.70%) | ≤ 5.00% | **PASS** | `classes_depth>=3 / classes` |
| 9 | 복잡도 과다(CC>=15) | 736/6095 (12.08%) | ≤ 15.00% | **PASS** | `cc>=15 / functions` |
| 10 | 롱메서드(len>=80) | 417/6095 (6.84%) | ≤ 8.00% | **PASS** | `len>=80 / functions` |
