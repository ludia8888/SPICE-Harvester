...
- Population: files **675**, functions **6365**, classes **903**, internal cross-imports **1601**

| # | Check | Ratio | Target | Status | Metric Basis |
| --- | --- | --- | --- | --- | --- |
| 1 | 계층 간 누수 | 0/1601 (0.00%) | <= 0.50% | **PASS** | `layer_leak_imports / internal_cross_imports` |
...
| 5 | 파일 응집도 저하 | 50/675 (7.41%) | <= 20.00% | **PASS** | `cohesion_risk_files / files` |
| 6 | 파일 단일 책임 위반 | 61/675 (9.04%) | <= 12.00% | **PASS** | `single_responsibility_risk_files / files` |
| 7 | 함수 단일 책임 위반 | 326/6365 (5.12%) | <= 10.00% | **PASS** | `(cc>=25 or len>=120) / functions` |
| 8 | 연속 상속 깊이(>=3) | 15/903 (1.66%) | <= 5.00% | **PASS** | `classes_depth>=3 / classes` |
| 9 | 복잡도 과다(CC>=15) | 734/6365 (11.53%) | <= 15.00% | **PASS** | `cc>=15 / functions` |
| 10 | 롱메서드(len>=80) | 414/6365 (6.50%) | <= 8.00% | **PASS** | `len>=80 / functions` |
...
- Longest functions: `mcp_servers/pipeline_mcp_server.py:202` (1321 lines), `mcp_servers/pipeline_mcp_server.py:204` (1272 lines), `shared/services/pipeline/pipeline_preflight_utils.py:685` (996 lines)
- Most complex functions: `shared/services/pipeline/pipeline_preflight_utils.py:685` (CC=292), `shared/services/pipeline/pipeline_definition_validator.py:151` (CC=249), `objectify_worker/main.py:1393` (CC=247)
