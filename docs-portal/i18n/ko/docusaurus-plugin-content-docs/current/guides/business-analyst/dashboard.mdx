...
이 가이드는 비즈니스 분석가가 Spice OS 데이터를 활용하여 대시보드(Dashboard)와 시각화(Visualization)를 구축하는 방법을 안내합니다. 검색 API(Search API), 프로젝션(Projections), 외부 도구를 활용하여 실행 가능한 데이터 뷰를 생성하는 방법을 배울 수 있습니다.
