...
이 가이드는 데이터 엔지니어가 외부 데이터 소스를 연결하고 Spice OS로 데이터를 가져오는 방법을 안내합니다. 연결(Connection) 구성, 가져오기 리소스 생성, 동기화 모드(Sync Mode) 선택, 데이터 수집 실행 방법을 학습합니다.
...
**연결(Connection)**은 외부 데이터 소스의 구성 및 자격 증명을 저장합니다.

### 연결 구조

각 연결에는 다음이 포함됩니다:
- **공개 구성** (호스트, 포트, 데이터베이스) -- `config_json`에 저장
- **시크릿** (비밀번호, 연결 문자열) -- `connector_connection_secrets`에 **AES-GCM 암호화**되어 저장
...
SQL 쿼리 기반 데이터 수집입니다. 구조화된 데이터에 대한 가장 일반적인 가져오기 유형입니다.
...
읽기 전용 쿼리 뷰 -- **SNAPSHOT 모드만 지원**합니다:
...
:::caution
가상 테이블은 SNAPSHOT 모드만 지원합니다. APPEND, UPDATE, INCREMENTAL 또는 CDC를 사용하려고 하면 오류가 발생합니다.
:::
...
**사용 시기:** 일일 보고서, 소규모 참조 테이블, 예측할 수 없이 변경되는 데이터에 사용합니다.

**동작:** 전체 대상 데이터셋이 쿼리 결과로 덮어쓰여집니다.
...
**사용 시기:** 이벤트 로그, 감사 추적, 행이 업데이트되지 않는 증가하는 데이터셋에 사용합니다.
...
**사용 시기:** 고객 프로필, 제품 카탈로그, 상태 테이블 -- 제자리에서 변경되는 모든 데이터에 사용합니다.
...
**사용 시기:** 신뢰할 수 있는 타임스탬프가 있는 대규모 테이블에서, 전체 테이블 스캔이 너무 비용이 많이 드는 경우에 사용합니다.
...
**사용 시기:** 실시간 복제, 소스 데이터베이스가 변경 추적을 지원하는 경우에 사용합니다.
...
1. **SNAPSHOT으로 시작하십시오** -- 가장 단순하고 신뢰할 수 있는 모드입니다. SNAPSHOT이 너무 느려질 때만 증분 모드로 전환하십시오.

2. **변경 가능한 데이터에 UPDATE를 사용하십시오** -- 소스에서 행이 변경되는 경우 UPDATE 모드가 기본 키로 올바르게 업서트를 처리합니다.

3. **의미 있는 표시 이름을 설정하십시오** -- 많은 가져오기가 있을 때 모니터링과 디버깅에 도움이 됩니다.

4. **먼저 연결을 테스트하십시오** -- 가져오기를 생성하기 전에 연결 테스트 엔드포인트를 사용하십시오.

5. **동기화 상태를 모니터링하십시오** -- `success_at`과 `error_count`를 확인하여 오래된 또는 실패하는 가져오기를 감지하십시오.

6. **암호화 키를 우아하게 로테이션하십시오** -- 다중 키 로테이션 기능을 통해 먼저 새 키를 추가하고, 모든 시크릿이 재암호화된 후 이전 키를 제거할 수 있습니다.

7. **점진적 롤아웃을 위해 허용 목록을 사용하십시오** -- 전역 활성화 전에 데이터베이스별로 JDBC/CDC 기능을 활성화하십시오.

8. **소스 쿼리를 최적화하십시오** -- WHERE 절과 워터마크 컬럼에서 사용되는 컬럼에 소스 데이터베이스의 인덱스를 추가하십시오.
