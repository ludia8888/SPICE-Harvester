...
이 가이드에서는 Spice OS에서 온톨로지(Ontology) 스키마를 정의하고 관리하는 방법을 다룹니다. 객체 유형(Object Type) 생성, 속성(Property) 구성, 링크 유형(Link Type) 설정, 그리고 스키마를 안전하게 발전시키는 방법을 알아볼까요.

## 개요

Spice OS에서 **온톨로지(Ontology)**는 데이터 모델을 설명하는 중앙 스키마 정의입니다. 외부 데이터베이스에서 가져온 데이터, 파이프라인(Pipeline)으로 변환된 데이터, 액션(Action)을 통해 생성된 데이터 등 플랫폼을 통해 흐르는 모든 데이터는 온톨로지 스키마를 따릅니다.
...
**객체 유형(Object Type)**은 관계형 데이터베이스의 테이블과 유사한 스키마 청사진입니다. 각 객체 유형은 온톨로지에 등록되며, 해당 유형의 모든 인스턴스(Instance)가 따라야 하는 구조를 정의합니다.
...
**속성(Property)**은 객체 유형에 정의된 이름과 타입을 가진 어트리뷰트입니다. 각 인스턴스가 어떤 데이터를 보유하는지 속성을 통해 정의합니다.
...
Spice OS는 XSD 기본 타입, 시맨틱 타입(Semantic Type), 복합 타입(Complex Type)을 아우르는 포괄적인 타입 시스템을 지원합니다.
...
시맨틱 타입은 기본 저장소에 매핑되면서도 도메인별 의미를 부여해 줍니다.
...
모든 객체 유형에는 인스턴스를 고유하게 식별하는 **기본 키(Primary Key)** 속성이 정확히 하나 있어야 합니다.
...
- 항상 필수이며 자동으로 인덱싱됩니다
- 명시적으로 설정하지 않으면 시스템이 자동 추론합니다 (`{type_id}_id` 또는 `id`를 탐색)
...
- 설정하지 않으면 `name` 속성이 기본값이 되며, 그다음으로 기본 키가 사용됩니다
- 키 스펙 레지스트리에도 함께 저장됩니다
...
**링크 유형(Link Type)**은 두 객체 유형 간의 방향성을 가진 관계를 정의합니다. 링크는 온톨로지의 일급 시민(First-Class Citizen)으로, 데이터 전반에 걸쳐 그래프 스타일의 탐색을 지원합니다.
...
비즈니스 요구사항이 변화하면 스키마도 함께 발전해야 합니다. Spice OS는 안전한 스키마 진화를 위한 도구와 안전장치를 제공합니다.
...
속성 데이터 타입을 변경할 때 시스템이 자동으로 호환성을 확인합니다.
...
Spice OS는 스키마 변경을 감지하기 위해 컬럼 정의의 안정적인 SHA-256 해시를 계산합니다. 이 해시는 다음과 같은 곳에서 활용됩니다.
...
객체 유형을 생성하거나 업데이트하면 시스템이 다단계 유효성 검증 파이프라인을 실행합니다.
...
모든 객체 유형은 하나 이상의 데이터셋으로 뒷받침될 수 있습니다. **백킹 데이터셋(Backing Dataset)**은 인스턴스로 객체화할 원시 데이터를 제공합니다.

### 데이터셋 연결

BFF 레이어를 통해 객체 유형을 생성할 때 다음 항목을 지정할 수 있습니다.
...
원시 데이터셋 행을 온톨로지 인스턴스로 변환하는 과정을 **객체화(Objectification)**라고 합니다. `ObjectifyRegistry`는 다음 정보를 추적합니다.
...
1. **의미 있는 기본 키를 선택하세요** -- 자동 증분 정수보다 비즈니스 식별자(`EMP-042`)를 사용하는 것이 좋습니다
2. **타이틀 키를 명시적으로 지정하세요** -- UI 컴포넌트가 의미 있는 이름을 표시하는 데 도움이 됩니다
3. **시맨틱 타입을 활용하세요** -- `string` 대신 `email`을 사용하면 타입별 유효성 검증과 렌더링이 가능해집니다
4. **제약 조건은 초기에 정의하세요** -- 나중에 제약 조건을 추가하면 기존 데이터가 거부될 수 있거든요
5. **공유 속성을 적극 활용하세요** -- 감사 필드(`createdAt`, `updatedAt`, `createdBy`) 같은 공통 패턴에 유용합니다
6. **카디널리티를 신중하게 설계하세요** -- `1:n`에서 `n:m`으로 변경하면 관계 구현 방식 자체가 달라집니다
7. **설명을 빠짐없이 작성하세요** -- 모든 객체 유형과 중요한 속성에 `description`을 포함해 주세요
8. **처음부터 i18n을 고려하세요** -- 단일 언어 배포라도 레이블에 `{ "en": "...", "ko": "..." }` 형식을 사용하는 것을 권장합니다

### 성능 고려사항

- **전략적으로 인덱싱하세요** -- 자주 검색되는 속성은 Elasticsearch용 `indexed`로 표시해 주세요
- **지나치게 넓은 스키마는 피하세요** -- 큰 유형은 링크 유형으로 연결된 집중된 유형으로 분할하는 것이 좋습니다
- **적절한 데이터 타입을 사용하세요** -- 숫자 데이터에는 `xsd:string`보다 `xsd:integer`가 더 효율적입니다
- **배열 속성은 적정 수준으로 유지하세요** -- 단일 속성에 큰 배열이 들어가면 직렬화 성능에 영향을 줄 수 있습니다

## 다음 단계

- **[파이프라인 빌더](./pipeline-builder)** -- 스키마에 맞춰 데이터를 변환하고 로드해 보세요
- **[가져오기 템플릿](./import-templates)** -- 외부 데이터 소스를 연결하는 방법을 확인하세요
- **[v2 Object Types API](/docs/api/v2-object-types)** -- 전체 API 레퍼런스
- **[핵심 개념](/docs/getting-started/concepts)** -- 기본 개념을 다시 살펴보세요
