...
이 가이드에서는 Spice OS에서 데이터 변환 파이프라인(Pipeline)을 빌드하고 테스트한 뒤 배포하는 과정을 다룹니다. 파이프라인은 원시 데이터를 구조화된 온톨로지 객체로 변환하는 핵심 메커니즘입니다.

## 개요

**파이프라인(Pipeline)**은 노드의 DAG(Directed Acyclic Graph, 방향성 비순환 그래프)로 정의되는 선언적 데이터 변환 워크플로입니다. 각 파이프라인은 다음 세 가지 요소로 구성됩니다.

- **입력 노드(Input Nodes)** -- 데이터셋, JDBC 연결, Kafka 토픽, S3 파일 등의 데이터 소스
- **변환 노드(Transform Nodes)** -- 데이터를 재구성하고 필터링하거나 보강 및 검증하는 연산
- **출력 노드(Output Nodes)** -- 데이터셋, 온톨로지 객체 유형, 지리시간 저장소 등의 출력 대상
...
파이프라인은 `nodes`, `edges`, 메타데이터를 포함한 JSON 문서로 정의합니다.
...
Spice OS는 네 가지 범주에 걸쳐 **38개 이상의 변환 유형**을 지원합니다.
...
키 컬럼별로 행을 그룹화합니다. 주로 집계와 함께 사용합니다.
...
UDF(User-Defined Function)를 사용하면 Python으로 작성한 사용자 정의 행별 변환을 **샌드박스 환경**에서 안전하게 실행할 수 있습니다.
...
| `global`/`nonlocal` | 스코프(Scope) 조작 불가 |
...
:::warning
UDF는 라이브러리를 가져오거나 루프를 사용하거나 파일 시스템에 접근할 수 없습니다. 이는 의도적인 설계인데요, UDF는 단순한 행별 변환이어야 하기 때문입니다. 복잡한 로직이 필요하다면 여러 변환 노드를 조합하는 방법을 고려해 보세요.
:::
...
**목적:** 부작용 없이 샘플 데이터로 빠르게 유효성을 검증합니다.
...
- 지원되지 않는 표현식은 오류 대신 `NULL`을 생성합니다(최선의 노력 방식)
- 전체 외래 키 유효성 검사는 건너뜁니다(전체 테이블 스캔이 필요하기 때문)
- 어떤 출력에도 데이터가 기록되지 않습니다
- 확인용 미리보기 결과를 반환합니다

**미리보기 사용 시기:** 리소스를 투입하기 전에 파이프라인 로직을 테스트하고, 표현식 유효성을 검사하고, 출력 스키마를 확인할 때 사용합니다.
...
보호된 브랜치(`main`, `prod`)에서 파이프라인을 변경하려면 배포 전에 **제안 승인(Proposal Approval)** 절차를 거쳐야 합니다.
...
파이프라인은 반복 실행을 위해 스케줄을 설정할 수 있습니다.
...
파이프라인은 여러 출력 유형에 데이터를 기록할 수 있습니다.
...
1. **변환은 집중적으로 유지하세요** -- 디버깅 편의를 위해 노드당 하나의 연산만 사용하세요
2. **필터링은 최대한 앞단에 배치하세요** -- 비용이 큰 연산(조인, 집계) 전에 필터를 먼저 적용하세요
3. **미리보기 모드를 적극 활용하세요** -- 빌드 전에 항상 미리보기로 테스트해 보세요
4. **스키마 기대값을 추가하세요** -- 데이터 품질 문제를 초기에 포착할 수 있습니다
5. **노드에 설명적인 이름을 붙이세요** -- 디버깅과 모니터링에 큰 도움이 됩니다

### 성능

1. **컬럼 선택을 최소화하세요** -- 데이터 볼륨을 줄이기 위해 초기에 `select`를 적용하세요
2. **조인 전에 중복을 제거하세요** -- 조인 출력 크기를 줄이는 데 효과적입니다
3. **증분 시맨틱을 활용하세요** -- 대규모 데이터셋의 전체 재처리를 피할 수 있습니다
4. **크로스 조인은 피하세요** -- O(n*m) 출력 행이 생성되기 때문입니다
5. **UDF는 단순하게 유지하세요** -- 복잡한 로직은 여러 변환 노드로 분리하는 것이 좋습니다

### 오류 처리

1. **적용 모드를 `fail`로 설정하세요** -- 문제가 전파되기 전에 포착할 수 있습니다
2. **required_columns 기대값을 추가하세요** -- 중요한 컬럼이 존재하는지 확인해 줍니다
3. **compute 전에 normalize를 사용하세요** -- 연산 전에 데이터를 정리하면 안정적입니다
4. **파이프라인 실행을 모니터링하세요** -- 실행 기록에서 실패와 경고를 주기적으로 확인하세요

## 다음 단계

- **[스키마 구성](./schema-config)** -- 대상 객체 유형을 정의해 보세요
- **[가져오기 템플릿](./import-templates)** -- 외부 데이터 소스를 연결하는 방법을 확인하세요
- **[파이프라인 도구 레퍼런스](/docs/reference/pipeline-tools)** -- 전체 변환 API 레퍼런스
