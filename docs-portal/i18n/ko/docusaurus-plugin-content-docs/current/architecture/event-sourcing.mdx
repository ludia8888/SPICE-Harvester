...
Spice OS는 코어 영속성 패턴으로 이벤트 소싱(Event Sourcing)을 사용합니다. 모든 상태 변경을 불변(immutable) 이벤트로 캡처해 완전한 감사 추적을 제공하며, 이벤트 리플레이, 타임 트래블 쿼리, 프로젝션 리빌드 같은 강력한 기능을 지원합니다.
...
Outbox relay는 outbox 테이블에서 커밋된 이벤트를 읽어 Kafka에 게시하는 백그라운드 프로세스입니다. 이 방식 덕분에 **DB 트랜잭션이 성공했을 때만 이벤트가 게시**되는 것을 보장할 수 있습니다.

### 2. Kafka 분산

이벤트는 Kafka 토픽(파티션)으로 게시됩니다. 파티셔닝 전략을 통해 특정 aggregate(객체)에 대한 이벤트가 순서대로 처리되는 것을 보장합니다.
...
모든 이벤트는 S3/MinIO 이벤트 스토어에 append-only 불변 로그로 저장됩니다. 날짜와 aggregate 기준으로 정리된 JSON 파일 형태입니다.
...
워커는 Kafka에서 이벤트를 소비해 각자 담당하는 리드 모델을 업데이트합니다.

**Event Processor**는 PostgreSQL 리드 모델에 최신 객체 상태를 반영합니다. 포인트 조회의 기본 리드 모델이기도 합니다.

**Index Syncer**는 Elasticsearch 인덱스를 업데이트하여 검색 결과를 최신으로 유지합니다. 인덱스 생성, 문서 업서트, 삭제를 처리합니다.

**Projection Builder**는 이벤트를 집계해 비정규화 뷰를 유지합니다. 여러 객체 유형의 데이터를 결합하거나 파생 값을 계산하는 역할을 합니다.
...
상태를 재구성할 때마다 전체 이벤트 히스토리를 리플레이하지 않아도 되도록, 플랫폼은 주기적으로 스냅샷을 생성합니다. 스냅샷은 특정 시퀀스 번호 시점의 aggregate 상태를 캡처한 것입니다.
...
이벤트 스토어에서 리드 모델을 완전히 재구성하려면 아래 명령어를 사용합니다.
...
특정 aggregate 또는 기간에 대해서만 이벤트를 리플레이할 수도 있습니다.
...
- **서로 다른 aggregate** 사이에는 순서가 보장되지 않습니다.
- aggregate ID 기반 Kafka 파티셔닝을 통해 토픽 내에서 aggregate 단위 순서를 보장합니다.
...
이벤트 처리에 실패하면 플랫폼은 재시도 후 데드 레터 전략을 따릅니다.

1. **즉시 재시도**: 컨슈머가 지수 백오프로 최대 3회 재시도합니다.
2. **지연 재시도**: 여전히 실패하면 지연 토픽으로 게시합니다(1분, 5분, 15분).
3. **데드 레터**: 모든 재시도가 실패하면 DLQ(`spice.events.dlq`)로 이동합니다.
4. **수동 처리**: 운영자가 Admin 서비스에서 DLQ 이벤트를 확인하고 리플레이하거나 폐기합니다.
