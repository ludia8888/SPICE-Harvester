...
from models.ontology import Relationship, OntologyBase
...
class RelationshipPathTracker:
    ...
    def __init__(self):
        ...
        self.default_weights = {
            # 강한 관계 (낮은 가중치)
            "hasParent": 0.5, "parentOf": 0.5,
            "hasChild": 0.5, "childOf": 0.5,
            "partOf": 0.3, "contains": 0.3,
            "instanceOf": 0.2, "hasInstance": 0.2,
            
            # 중간 관계
            "worksFor": 1.0, "hasEmployee": 1.0,
            "belongsTo": 1.0, "has": 1.0,
            "manages": 1.2, "managedBy": 1.2,
            
            # 약한 관계 (높은 가중치)
            "related": 2.0, "associated": 2.0,
            "similar": 2.5, "mentions": 3.0
        }
        ...
        for ontology in ontologies:
            ...
            for rel in ontology.relationships:
                ...
                if rel.inverse_predicate:
                    ...
                    inverse_cardinality_modifier = self.cardinality_weights.get(inverse_cardinality, 1.0)
    ...
    def find_shortest_path(self, start: str, end: str, max_depth: int = 5) -> Optional[RelationshipPath]:
        """최단 경로 탐색 (단순 버전)"""
    ...
    def find_all_reachable_entities(self, start: str, max_depth: int = 3) -> Dict[str, RelationshipPath]:
    ...
    def _calculate_semantic_score(self, path: RelationshipPath, query: PathQuery) -> float:
        ...
        # predicate 의미적 강도
        semantic_strengths = {
            "instanceOf": 0.9, "hasInstance": 0.9,
            "partOf": 0.8, "contains": 0.8,
            "parentOf": 0.7, "childOf": 0.7,
            "worksFor": 0.6, "hasEmployee": 0.6,
            "related": 0.3, "associated": 0.3
        }
    ...
    def _get_inverse_cardinality(self, cardinality: str) -> str:
        ...
        inverse_map = {
            "1:1": "1:1", "1:n": "n:1", "n:1": "1:n", "n:n": "n:n",
            "one": "many", "many": "one"
        }
    ...
    def export_graph_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "relationship_types": list(set(
                hop.predicate for hops in self.relationship_graph.values() for hop in hops
            )),
            "cardinality_distribution": self._get_cardinality_distribution(),
            "average_connections_per_entity": sum(len(hops) for hops in self.relationship_graph.values()) / len(self.entities) if self.entities else 0
        }
