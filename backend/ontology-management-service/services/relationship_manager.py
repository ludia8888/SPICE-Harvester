...
from models.ontology import Relationship, MultiLingualText, Cardinality
...
class RelationshipManager:
    ...
    def _generate_inverse_relationship(
        ...
        # 역관계 predicate 결정
        inverse_predicate = (
            forward_relationship.inverse_predicate or 
            self._generate_inverse_predicate(forward_relationship.predicate)
        )
        ...
        inverse_description = self._generate_inverse_description(
            forward_relationship.description,
            source_class,
            target_class
        )
    ...
    def _generate_inverse_label(
        ...
        if isinstance(forward_label, str):
            return MultiLingualText.from_string(
                self._invert_label_text(forward_label),
                'ko'
            )
    ...
    def _generate_inverse_description(
        self,
        forward_description: Optional[MultiLingualText],
        source_class: str,
        target_class: str
        ...
        if isinstance(forward_description, str):
            return MultiLingualText.from_string(
                f"Inverse of: {forward_description}",
                'en'
            )
    ...
    def detect_relationship_conflicts(self, relationships: List[Relationship]) -> List[str]:
        ...
        for rel in relationships:
            ...
            if rel.predicate in predicate_map:
                conflicts.append(
                    f"Duplicate predicate '{rel.predicate}' found in relationships"
                )
            ...
            if rel.target == rel.predicate:
                conflicts.append(
                    f"Self-referential relationship detected: {rel.predicate}"
                )
    ...
    def generate_relationship_summary(self, relationships: List[Relationship]) -> Dict[str, Any]:
        ...
        return {
            ...
            "inverse_coverage": f"{inverse_coverage}/{len(relationships)} ({inverse_coverage/len(relationships)*100:.1f}%)" if relationships else "0/0 (0%)",
