...
graph TD
    ...
    subgraph "EventPublisher (message_relay)"
        D[EventPublisher] -->|3. Tail indexes/by-date| C
        D -->|4. Kafka 발행| E[Kafka Event Bus]
    end

    subgraph "쓰기 모델 (Instance Worker)"
        F(Instance Worker) -->|5. Command 구독| E
        F -->|6. Claim(멱등+순서)<br/>processed_events| P[PostgreSQL Registry]
        F -->|7. TerminusDB 상태 업데이트<br/>(Write-Model Cache)| H[TerminusDB]
        F -->|8. Append Domain Event| C
    end

    subgraph "읽기 모델 (Projection Worker)"
        I(Projection Worker) -->|9. Event 구독| E
        I -->|10. Claim(멱등+순서)<br/>processed_events| P
        I -->|11. ES 문서 생성/수정<br/>(Read Model)| J[Elasticsearch]
    end
...
2.  **Command 발행 (at EventPublisher)**
    -   `EventPublisher` 서비스가 Event Store의 `indexes/by-date/...`를 tail 합니다.
    -   새 커맨드 이벤트를 Kafka의 `INSTANCE_COMMANDS_TOPIC` 토픽으로 발행합니다.
...
3.  **Command 처리 (at Instance Worker)**
    ...
    -   **[핵심 동작 1] 멱등/순서 클레임**: `processed_events(handler,event_id)`로 **claim(lease+heartbeat)** 해서 동일 이벤트가 시스템 전체에서 1회만 side-effect를 만들도록 보장합니다.
    -   **[핵심 동작 2] TerminusDB 상태 업데이트**: TerminusDB에 해당 인스턴스의 최신 상태 문서를 생성/업데이트합니다.
    -   **[핵심 동작 3] 도메인 이벤트 append**: 모든 쓰기 작업이 성공하면, `INSTANCE_CREATED` 도메인 이벤트를 `EventEnvelope(kind=domain)`로 Event Store에 append 합니다. EventPublisher가 이를 Kafka로 전달합니다.
...
4.  **프로젝션 (at Projection Worker)**
    ...
    -   `processed_events`로 중복/재시도/재전달에도 idempotent 하게 처리합니다.
