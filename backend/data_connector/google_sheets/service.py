...
class GoogleSheetsService:
    ...
    async def preview_sheet(self, sheet_url: str) -> GoogleSheetPreviewResponse:
        ...
        if ' ' in worksheet_name or '/' in worksheet_name or '(' in worksheet_name or ')' in worksheet_name:
    ...
    async def _get_sheet_data(
        ...
        try:
            ...
            if e.response.status_code == 403:
                raise ValueError(
                    "Cannot access the Google Sheet data. "
                    "Please ensure it's shared publicly."
                )
    ...
    async def _start_polling(self, sheet_id: str):
        ...
        while registered_sheet.is_active:
            try:
                ...
                # Get current data
                data = await self._get_sheet_data(
                    sheet_id, 
                    registered_sheet.worksheet_name
                )
