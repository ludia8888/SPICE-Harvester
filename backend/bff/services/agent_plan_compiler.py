...
def _build_plan_system_prompt() -> str:
    return (
        ...
        "- If a pipeline definition_json is required, include a VALID graph (nodes/edges) that passes server validation: at least one output node, transform nodes with required operation metadata, and joins with leftKey/rightKey or leftKeys/rightKeys (or joinKey).\n"
        ...
        "  - join: {operation:\"join\", leftKey:\"col\", rightKey:\"col\"} or {leftKeys:[...], rightKeys:[...]} with joinType:\"inner|left|right|full|cross\", allowCrossJoin:false\n"
