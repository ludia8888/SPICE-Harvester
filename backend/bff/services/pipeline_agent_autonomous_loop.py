...
class AutonomousPipelineAgentDecision(BaseModel):
    ...
    # Compatibility single-call shape (kept for backward compatibility with older mocks/models).
...
def _build_system_prompt(*, allowed_tools: List[str]) -> str:
    ...
    return (
        ...
        "  f) `ontology_create()` — persist to ontology registry\n"
        "  g) `ontology_register_object_type(class_id='Customer', dataset_id='...', primary_key=['customer_unique_id'], title_key=['customer_unique_id'], "
        "property_mappings=[{'source_field': 'col1', 'target_field': 'prop1'}, ...], auto_sync=true)` — register with Foundry-style integrated backing_source\n"
        ...
        "- `ontology_create()` — db_name/branch auto-injected. Persists to ontology registry.\n"
        "- `ontology_register_object_type(class_id='Customer', dataset_id='...', primary_key=['customer_unique_id'], title_key=['customer_unique_id'], "
        "property_mappings=[{source_field: 'col', target_field: 'prop'}, ...], auto_sync=true)` — db_name auto-injected. "
        "property_mappings stores column→property mappings directly in OMS backing_source (Foundry-style). No separate create_mapping_spec needed.\n"
        "\n"
        "After ALL classes are registered (with property_mappings):\n"
        "  h) `objectify_run(db_name='...', dataset_id='...', target_class_id='Customer')` — uses OMS backing_source mappings directly\n"
        "  i) `objectify_wait(...)` → `ontology_query_instances(...)` to verify\n"
        "\n"
...
def _get_item_type(item_json: str) -> str:
    ...
    try:
        ...
        logging.getLogger(__name__).warning("Exception fallback at bff/services/pipeline_agent_autonomous_loop.py:1408", exc_info=True)
        return "unknown"
...
def _reshrink_tool_output(item_json: str, max_chars: int) -> str:
    ...
    try:
        ...
        logging.getLogger(__name__).warning("Exception fallback at bff/services/pipeline_agent_autonomous_loop.py:1416", exc_info=True)
    ...
    if isinstance(content, list) and content:
        ...
        for text in texts:
            try:
                ...
                logging.getLogger(__name__).warning("Exception fallback at bff/services/pipeline_agent_autonomous_loop.py:1773", exc_info=True)
    ...
    *,
    ...
    for step_idx in range(max_steps):
        ...
        if decision.action == "finish":
            if isinstance(state.plan_obj, dict):
                try:
                    ...
                    logging.getLogger(__name__).warning("Exception fallback at bff/services/pipeline_agent_autonomous_loop.py:2690", exc_info=True)
        ...
        for call in tool_calls[:max_tool_calls_per_step]:
            ...
            try:
                ...
                logging.getLogger(__name__).warning("Exception fallback at bff/services/pipeline_agent_autonomous_loop.py:2886", exc_info=True)
    ...
    if isinstance(state.plan_obj, dict):
        try:
            ...
            logging.getLogger(__name__).warning("Exception fallback at bff/services/pipeline_agent_autonomous_loop.py:3129", exc_info=True)
            yield StreamEvent(
                event_type="complete",
                data={
