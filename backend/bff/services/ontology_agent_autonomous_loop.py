...
def _build_system_prompt(*, allowed_tools: List[str]) -> str:
    ...
    return (
        ...
        "\n"
        "CRITICAL - Tool parameter names (use these EXACT names):\n"
        "- ontology_new: class_id (required), label (required), description (optional)\n"
        "- ontology_add_property: name (required), type (required, e.g. xsd:string), label (required), required, primary_key, title_key\n"
        "- ontology_add_relationship: predicate (required), target (required), label (required), cardinality (1:1, 1:n, n:1, n:m)\n"
        "- ontology_infer_schema_from_data: columns (required, array of strings), data (required, array of arrays)\n"
        "- ontology_create/ontology_update: db_name (auto-injected), branch (auto-injected)\n"
        "\n"
        "Common patterns:\n"
        ...
        "\n"
        "IMPORTANT: Do NOT call ontology_create/ontology_update unless explicitly asked to save to database.\n"
        "For building ontology in memory only, use: ontology_new -> ontology_add_property -> ontology_validate -> ontology_preview -> finish\n"
        "\n"
        f"Available tools:\n{tool_lines}\n"
        "\n"
        "Schema:\n"
