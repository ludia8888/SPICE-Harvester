...
def create_lakefs_storage_service(settings: ApplicationSettings) -> Optional[LakeFSStorageService]:
    ...
    endpoint = settings.storage.lakefs_s3_endpoint_effective
    access = (settings.storage.lakefs_access_key_id or "").strip()
    secret = (settings.storage.lakefs_secret_access_key or "").strip()
