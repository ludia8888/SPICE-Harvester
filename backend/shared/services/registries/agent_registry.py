...
class AgentRegistry(PostgresSchemaRegistry):
    ...
    async def _ensure_tables(self, conn: asyncpg.Connection) -> None:  # type: ignore[override]
        ...
        await conn.execute(f"CREATE INDEX IF NOT EXISTS idx_agent_runs_plan_id ON {self._schema}.agent_runs(plan_id)")
        await conn.execute(f"CREATE INDEX IF NOT EXISTS idx_agent_runs_status ON {self._schema}.agent_runs(status)")
        await conn.execute(f"CREATE INDEX IF NOT EXISTS idx_agent_runs_tenant ON {self._schema}.agent_runs(tenant_id)")
        ...
        await conn.execute(f"CREATE INDEX IF NOT EXISTS idx_agent_steps_run_id ON {self._schema}.agent_steps(run_id)")
        await conn.execute(f"CREATE INDEX IF NOT EXISTS idx_agent_steps_tool_id ON {self._schema}.agent_steps(tool_id)")
        await conn.execute(f"CREATE INDEX IF NOT EXISTS idx_agent_steps_tenant ON {self._schema}.agent_steps(tenant_id)")
