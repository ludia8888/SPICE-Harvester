...
THINK ULTRA³ - Intelligent retry with exponential backoff
...
class FailedMessage:
    ...
    def is_poison(self) -> bool:
        ...
        # Poison if certain error patterns are detected
        poison_patterns = [
...
class DLQHandler:
    """
    Handles Dead Letter Queue processing with intelligent retry
    ...
    def __init__(
        ...
        consumer_group: Optional[str] = None  # FIXED: Make consumer group configurable
    ):
        """
        ...
        Args:
            ...
            poison_topic: Topic for poison messages (unretrievable)
            consumer_group: Consumer group ID (default: 'dlq-handler-group')
        """
        ...
        # Thread pool for blocking operations (FIXED)
        self.executor = ThreadPoolExecutor(max_workers=2)
        
        # Retry queue (in-memory for performance)
    ...
    async def start_processing(self):
        ...
        consumer_config['group.id'] = self.consumer_group  # FIXED: Use configurable group
        ...
        logger.info("Started DLQ processing")
    ...
    async def _process_loop(self):
        ...
        while self.processing:
            try:
                ...
                # Poll for messages in thread pool (non-blocking for event loop)
    ...
    async def _retry_message(self, failed_msg: FailedMessage):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Retry {failed_msg.retry_count} failed for message: {process_error}")
    ...
    async def _add_to_retry_queue(self, failed_msg: FailedMessage):
        ...
        await self.redis_client.expire(f"dlq:retry:{failed_msg.message_id}", 86400)  # 24 hour TTL
    ...
    async def _move_to_poison_queue(self, failed_msg: FailedMessage):
        """
        Move message to poison queue (unretrievable)
        
        Args:
            failed_msg: Message to move
        """
        try:
            poison_data = {
                'message_id': failed_msg.message_id,
            ...
            await self.redis_client.ltrim("dlq:poison:messages", 0, 999)  # Keep last 1000
    ...
    async def _record_recovery(self, failed_msg: FailedMessage):
        ...
        await self.redis_client.ltrim("dlq:recovered:messages", 0, 99)  # Keep last 100
    ...
    async def _check_circuit_breaker(self) -> bool:
        ...
        Returns:
            True if circuit breaker is open (stop processing)
        """
        if not self.circuit_breaker_open:
            return False
        
        # Check if it's time to half-open the circuit
    ...
    async def _handle_circuit_breaker_failure(self):
        ...
        if self.circuit_breaker_failures >= self.circuit_breaker_threshold:
            ...
            self.circuit_breaker_reset_time = datetime.now(timezone.utc) + timedelta(minutes=1)
            logger.error(f"Circuit breaker opened! Will retry at {self.circuit_breaker_reset_time}")
    ...
    async def get_poison_messages(self, limit: int = 10) -> List[Dict[str, Any]]:
        """Get recent poison messages for analysis"""
        messages = await self.redis_client.lrange("dlq:poison:messages", 0, limit - 1)
        return [json.loads(msg) for msg in messages]


# Convenience function to create DLQ handler
