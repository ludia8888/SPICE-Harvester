...
class SmokeContext:
    ...
    def instance_aggregate_id(self) -> str:
        # Instance write-side endpoints default to the platform's default branch (Foundry parity: main).
...
async def _build_plan(op: Operation, ctx: SmokeContext) -> RequestPlan:
    ...
    if key == ("POST", "/api/v2/datasets/{datasetRid}/branches"):
        url = f"{BFF_URL}{_format_path(op.path, ctx, overrides={'datasetRid': 'ri.spice.main.dataset.00000000-0000-0000-0000-000000000000'})}"
        body = {"name": f"smoke-{uuid.uuid4().hex[:6]}", "sourceBranchName": "main"}
    ...
    if key == ("POST", "/api/v2/datasets/{datasetRid}/files/{filePath}/upload"):
        ...
        return RequestPlan(op.method, op.path, url, (200, 201, 400, 404, 409), params={"branchName": "main"}, data=b"id,name\n1,smoke\n", headers=headers)
    ...
    if key == ("GET", "/api/v2/datasets/{datasetRid}/readTable"):
        ...
        return RequestPlan(
            ...
            url,
            (200, 400, 404),
            params={"branchName": "main", "format": "CSV", "rowLimit": 10},
        )

    if key == ("GET", "/api/v2/datasets/{datasetRid}/getSchema"):
        url = f"{BFF_URL}{_format_path(op.path, ctx, overrides={'datasetRid': 'ri.spice.main.dataset.00000000-0000-0000-0000-000000000000'})}"
        return RequestPlan(op.method, op.path, url, (200, 404, 400), params={"preview": "true", "branchName": "main"})
    ...
    if key == ("GET", "/api/v2/datasets/{datasetRid}/transactions"):
        ...
        return RequestPlan(
            ...
            url,
            (200, 404, 400),
            params={"preview": "true", "branchName": "main", "pageSize": 10},
        )
    ...
    if key == ("PUT", "/api/v2/datasets/{datasetRid}/putSchema"):
        ...
        return RequestPlan(op.method, op.path, url, (204, 400, 404, 409), params={"preview": "true", "branchName": "main"}, json_body=body)
    ...
    if key == ("POST", "/api/v2/datasets/{datasetRid}/transactions"):
        ...
        return RequestPlan(op.method, op.path, url, (200, 201, 400, 404, 409, 422), params={"branchName": "main"}, json_body=body)
