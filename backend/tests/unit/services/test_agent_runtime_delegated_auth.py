...
async def test_agent_runtime_sends_service_token_and_delegated_user_token(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert headers.get("Idempotency-Key") is None
    assert UUID(str(headers.get("X-Agent-Tool-Run-ID")))


@pytest.mark.unit
@pytest.mark.asyncio
async def test_agent_runtime_forwards_idempotency_and_agent_session_headers(monkeypatch: pytest.MonkeyPatch) -> None:
    import agent.services.agent_runtime as module
    ...
    assert headers.get("X-Agent-Session-ID") == "session-1"
