...
def test_core_foundry_conflict_system():
    ...
    try:
        ...
        from conflict_converter import ConflictConverter, ConflictSeverity, PathType
        ...
        terminus_conflicts = [
            ...
            },
            {
                ...
                "source_change": {
                    "type": "add",
                    "new_value": "http://example.org/NewParentClass"
                },
                "target_change": {
                    "type": "add",
        ...
        for i, conflict in enumerate(foundry_conflicts, 1):
            ...
            assert conflict['type'] in ['modify_modify_conflict', 'add_add_conflict', 'delete_modify_conflict']
    ...
    try:
        ...
        for expected in expected_endpoints:
            found = any(expected.replace('{db_name}', '{db_name}') in endpoint for endpoint in merge_endpoints)
            if found:
                print(f"   ✅ {expected}")
            else:
